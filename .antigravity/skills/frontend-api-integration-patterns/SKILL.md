...
// VOCAMON SPECIFIC: Handle .NET ApplicationResult pattern
export const vocamonClient = async (url, options = {}) => {
  const result = await apiClient(url, options);
  if (result && 'isSuccess' in result) {
    if (!result.isSuccess) throw new ApiError(result.error || "Operation failed", 400, result);
    return result.data;
  }
  return result;
};
