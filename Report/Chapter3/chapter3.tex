...
Việc bảo vệ các tài nguyên nhạy cảm của hệ thống được giải quyết thông qua mô hình phân quyền dựa trên vai trò (Role-Based Access Control — RBAC). Thay vì kiểm tra quyền tại từng phương thức riêng lẻ, ASP.NET Core cho phép khai báo chính sách bảo mật tập trung ngay tại cấp độ lớp bằng thuộc tính \texttt{[Authorize(Roles = "...")]}, sau đó Middleware Pipeline xác thực JWT tự động thực thi chính sách này trước khi bất kỳ luồng nghiệp vụ nào được khởi chạy \cite{MiddlewareDocs2024}.
...
\begin{table}[H]
\centering
\caption{Ánh xạ quyền truy cập theo vai trò lên các nhóm Controller.}
\label{tab:rbac}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Nhóm Controller} & \textbf{User} & \textbf{Admin} & \textbf{SuperAdmin} \\ \hline
Learning, Battle, Pet, Quest (User) & \checkmark & & \\ \hline
VocabularySet, Achievement (Admin) & & \checkmark & \checkmark \\ \hline
GymLeader (Admin) & & \checkmark & \checkmark \\ \hline
Quests \& Achievements (Admin) & & \checkmark & \checkmark \\ \hline
System Config, Health, Maintenance & & & \checkmark \\ \hline
\end{tabular}
\end{table}

Thông tin vai trò được ASP.NET Core Identity ghi vào JWT dưới dạng claim chuẩn tại thời điểm đăng nhập. Phía ứng dụng quản trị, thông tin này được đọc trực tiếp từ token bằng thư viện \texttt{jwtDecode} nhằm điều chỉnh động giao diện điều hướng — người dùng với vai trò \textbf{SuperAdmin} sẽ thấy thêm mục System Health so với \textbf{Admin} thông thường. Mọi yêu cầu vào hệ thống đều phải thông qua lớp kiểm tra claim này; bất kỳ token nào không thuộc hai vai trò trên sẽ bị từ chối ngay tại bước khởi tạo phiên.
...
\begin{figure}[H]
\centering
\includegraphics[width=14cm]{images/FinalDB.png}
\caption{Lược đồ ERD nhóm quản lý người dùng và xác thực.}
\label{fig_erd_users}
\end{figure}

Bảng \textbf{Users} là điểm hội tụ trung tâm của toàn bộ lược đồ, lưu trữ định danh, thông tin bảo mật và các thông số tăng trưởng của người học gồm điểm kinh nghiệm (XP), điểm thành tích (AP), chuỗi rèn luyện (StreakCount), số gợi ý còn lại (HintBalance) và cấp độ hiện tại (Level). Bảng \textbf{RefreshTokens} quản lý vòng đời của các mã làm mới JWT, hỗ trợ cơ chế luân phiên token an toàn. Bảng \textbf{Notifications} ghi nhận các sự kiện thông báo đẩy thời gian thực với trường \texttt{IsRead} và liên kết khóa ngoại chặt chẽ tới \textbf{Users}, đảm bảo tính toàn vẹn khi xóa tài khoản theo quy tắc Cascade Delete.
...
Bảng \textbf{Vocabularies} chi tiết hóa các đặc tính ngôn ngữ của từng mục từ, bao gồm phiên âm IPA, nghĩa, câu ví dụ, cấp độ CEFR, từ loại, URL hình ảnh minh họa và đường dẫn tệp âm thanh được tổng hợp bởi Azure Speech Services. Bảng \textbf{VocabularySets} đóng vai trò là các "container" tri thức, định danh chủ đề (Theme), ngưỡng cấp độ yêu cầu và trạng thái công khai. Mối quan hệ nhiều-nhiều giữa hai thực thể này được giải quyết thông qua bảng trung gian \textbf{SetVocabularies}. Bảng \textbf{UserVocabularySets} ghi nhận việc người dùng đăng ký sử dụng bộ từ, đồng thời lưu số lần hoàn thành (Milestone) — thông số then chốt điều phối xác suất phần thưởng thú ảo hiếm theo cơ chế leo thang.
...
Bảng \textbf{LearningSessions} theo dõi từng phiên tương tác với các trường trạng thái, loại phiên (học mới hoặc ôn tập), thời gian bắt đầu và kết thúc. Mỗi câu trả lời được ghi nhận chi tiết trong \textbf{AnswerRecords} với các trường \texttt{IsCorrect}, \texttt{ElapsedMs} và loại câu hỏi, cung cấp nguồn dữ liệu thô cho các phân tích hiệu suất sau này. Hai bảng cốt lõi cho thuật toán SM-2 là \textbf{UserVocabularyProgresses} và \textbf{VocabularyReviewHistories}: bảng trước lưu trạng thái tức thời của từng từ vựng bao gồm chỉ số Easiness Factor (EF), khoảng cách ôn tập hiện tại (Interval), số lần lặp (RepetitionCount) và thời điểm ôn tập kế tiếp (NextReviewTime); bảng sau giữ vai trò nhật ký dài hạn, giúp phân tích xu hướng ghi nhớ và phát hiện các mẫu quên lãng theo thời gian.
...
\centering
\includegraphics[width=14cm]{images/FinalDB.png}
\caption{Lược đồ ERD nhóm Gamification, thú ảo và hệ thống nhiệm vụ.}
\label{fig_erd_gamification}
\end{figure}

Bảng \textbf{Pets} định nghĩa mẫu thiết kế của các sinh vật ảo với các thuộc tính PetType, PetRarity, DropRate và chuỗi tiến hóa (NextEvolutionId tự tham chiếu). Bảng \textbf{UserOwnedPets} thể hiện quan hệ sở hữu, lưu trữ cấp độ, XP hiện tại, trạng thái kích hoạt và chỉ số sinh tồn của từng thú ảo. Hệ thống vật phẩm được quản lý qua cặp bảng \textbf{Items} và \textbf{UserItems}, trong đó \textbf{UserItems} áp dụng ràng buộc duy nhất trên cặp (UserId, ItemId) để đảm bảo tính nhất quán tồn kho. Các bảng \textbf{DailyQuests} và \textbf{UserDailyQuests} điều phối hệ thống nhiệm vụ hàng ngày với trạng thái hoàn thành và nhận thưởng; cặp \textbf{Achievements} và \textbf{UserAchievements} theo dõi tiến độ thành tích dài hạn thông qua trường \texttt{ProgressValue} so sánh với ngưỡng \texttt{ConditionValue}.
...
Dữ liệu chiến đấu được phân rã thành ba cấp độ chi tiết tăng dần. Bảng \textbf{BattleSessions} đại diện cho một phiên đấu đầy đủ, lưu trữ loại đối kháng (PvP/PvE), trạng thái, điểm ELO thay đổi và thời điểm kết thúc. Bảng \textbf{BattleRounds} ghi nhận từng vòng câu hỏi với bộ từ vựng sử dụng và kết quả vòng đó. Bảng \textbf{BattlePetStates} duy trì trạng thái động của từng thú ảo trong mỗi vòng đấu, bao gồm HP hiện tại và thuộc tính hệ — thiết kế này cho phép khôi phục chính xác trạng thái nếu kết nối WebSocket bị gián đoạn và phải tái kết nối. Đối với chế độ PvE, bảng \textbf{GymLeaders} định nghĩa thông số của các đối thủ hệ thống với bộ từ vựng và ngưỡng HP riêng biệt theo từng cấp thử thách.
...
Song song với ứng dụng học tập dành cho người dùng đầu cuối, hệ thống triển khai một ứng dụng quản trị độc lập trên nền tảng Next.js (App Router) — \textbf{wordsoul-admin} — hoạt động như một giao diện điều hành tập trung cho đội ngũ vận hành. Việc tách biệt ứng dụng quản trị khỏi ứng dụng học tập không chỉ giảm thiểu bề mặt tấn công mà còn cho phép triển khai độc lập và thiết lập chính sách bảo mật riêng biệt theo từng ứng dụng.

Quy trình xác thực được thực thi theo hai tầng. Tại tầng máy chủ (Backend), \texttt{AdminController} và các controller quản trị liên quan được bảo vệ bởi thuộc tính \texttt{[Authorize(Roles = "Admin,SuperAdmin")]}. Tại tầng giao diện (Frontend), \texttt{AuthContext} đọc JWT được lưu trong Cookie của trình duyệt, giải mã claim vai trò và kiểm tra ngay tại khởi động ứng dụng — bất kỳ token nào không mang vai trò \textbf{Admin} hoặc \textbf{SuperAdmin} sẽ bị xóa Cookie và chuyển hướng về trang đăng nhập. Để duy trì phiên làm việc không gián đoạn, luồng làm mới token (Refresh Token Rotation) được tích hợp trực tiếp vào Axios interceptor: khi nhận phản hồi HTTP 401, hệ thống tự động gọi endpoint \texttt{/auth/refresh-token}, cập nhật Access Token mới và thực hiện lại yêu cầu gốc mà không yêu cầu quản trị viên đăng nhập lại.

Giao diện điều hướng của ứng dụng được thiết kế theo nguyên tắc hiển thị điều kiện dựa trên vai trò. Thanh bên (Sidebar) hiển thị năm mục điều hướng dành cho tất cả quản trị viên: Bảng điều khiển (Dashboard), Quản lý người dùng, Kho từ vựng, Nhiệm vụ \& Thành tích, và Vận hành Gym. Mục thứ sáu là \textbf{System Health \& Config} chỉ xuất hiện với các tài khoản mang vai trò \textbf{SuperAdmin}, cung cấp giao diện trực tiếp để chỉnh sửa tham số hệ thống, xem trạng thái cơ sở dữ liệu và thực thi các lệnh bảo trì hạ tầng như xả bộ nhớ đệm Redis.

\begin{figure}[H]
\centering
\includegraphics[width=15cm]{images/Final_IF_Static.png}
\caption{Giao diện bảng điều phối quản trị viên (wordsoul-admin).}
\label{fig:ch3_admin}
\end{figure}
...
Khi quản trị viên nhập danh sách từ vựng thô vào hệ thống, \texttt{GeminiAiService} sẽ gọi Gemini API (mô hình \texttt{gemini-2.5-flash}) để sinh tự động toàn bộ metadata bao gồm phiên âm IPA, nghĩa tiếng Việt, câu ví dụ ngữ cảnh, phân loại từ loại và cấp độ CEFR tương ứng. Nhằm tuân thủ giới hạn tần suất của API, dữ liệu được xử lý theo từng lô (batch) với kích thước mặc định là 10 từ và khoảng dừng 5.000 ms giữa các lô, đảm bảo tính ổn định khi xử lý danh sách từ vựng lớn. Song song với việc sinh metadata, \texttt{UnsplashService} truy vấn Unsplash API để lấy URL hình ảnh minh họa phù hợp theo ngữ nghĩa của từng từ, giúp kích thích khả năng liên tưởng hình ảnh trong quá trình học tập.

Điểm then chốt về hiệu năng trong luồng này là lớp \texttt{VocabularyAiCacheService}, được triển khai dựa trên \texttt{IDistributedCache} — giao diện tiêu chuẩn của ASP.NET Core ánh xạ tới Redis trong môi trường thực tế. Mỗi kết quả AI cho một từ vựng được lưu cache với khóa có tiền tố \texttt{vocab-ai-preview:} và thời gian tồn tại (TTL) là 7 ngày. Chiến lược này cho phép hệ thống bỏ qua hoàn toàn lần gọi Gemini API đối với các từ đã được xử lý trước đó, giảm thiểu chi phí vận hành và độ trễ phản hồi đáng kể. Quan trọng hơn, lớp cache được thiết kế theo nguyên tắc \textit{graceful degradation}: nếu Redis không khả dụng, luồng xử lý vẫn tiếp tục bình thường bằng cách gọi Gemini API trực tiếp, tránh tình trạng cascading failure.
...
Luồng xử lý được vận hành bởi \texttt{AzureSpeechService}: khi một từ vựng mới được thêm vào hệ thống, phương thức \texttt{SynthesizeAndUploadAsync} gọi Azure Cognitive Services với giọng đọc \texttt{en-US-AndrewNeural} và định dạng đầu ra MP3 (Audio16Khz32KBitRateMonoMp3). Tệp âm thanh sau khi được tổng hợp sẽ được tải lên Azure Blob Storage và URL công khai sẽ được ghi vào trường \texttt{AudioUrl} của bản ghi \textbf{Vocabularies} trong cơ sở dữ liệu. Trong phiên học, khi người dùng gặp câu hỏi dạng Listening, Frontend trực tiếp phát tệp MP3 từ URL Blob Storage — không cần gọi lại API tổng hợp, đảm bảo độ trễ phát âm thanh ở mức tối thiểu.
...
Mỗi khi người dùng đăng nhập vào ngày mới, \texttt{DailyQuestService} kiểm tra xem nhiệm vụ trong ngày đã được tạo chưa — nếu chưa, hệ thống sẽ sinh ngẫu nhiên một tập hợp nhiệm vụ từ danh sách các mẫu đang kích hoạt (Active Quest Templates) và gán cho người dùng. Mỗi nhiệm vụ có loại phần thưởng riêng biệt: XP, vật phẩm hoặc thú ảo. Khi người dùng đạt đủ tiến độ và nhận thưởng, \texttt{ClaimRewardAsync} điều phối luồng xử lý phần thưởng tương ứng thông qua các service chuyên biệt.

Song song với nhiệm vụ ngắn hạn, \texttt{UserAchievementService} theo dõi tiến độ thành tích dài hạn. Phương thức \texttt{UpdateAchievementProgressAsync} được gọi sau mỗi sự kiện nghiệp vụ quan trọng — hoàn thành phiên học, mở khóa thú ảo, đạt chuỗi rèn luyện — nhằm cập nhật \texttt{ProgressValue} cho tất cả thành tích có \texttt{ConditionType} tương ứng. Khi tiến độ vượt ngưỡng, thành tích được đánh dấu hoàn thành và người dùng có thể nhận vật phẩm phần thưởng. Toàn bộ các thao tác cập nhật thành tích được bọc trong giao dịch cơ sở dữ liệu (\texttt{BeginTransactionAsync}) để đảm bảo tính nguyên tử.

Hệ thống tồn kho được bảo vệ khỏi tranh chấp đồng thời (race condition) thông qua cơ chế khóa cấp người dùng trong \texttt{UserInventoryService}: mỗi \texttt{userId} được ánh xạ tới một \texttt{SemaphoreSlim(1,1)} riêng biệt. Điều này đảm bảo rằng khi nhiều yêu cầu thêm/bớt vật phẩm xảy ra đồng thời cho cùng một người dùng — ví dụ hoàn thành nhiệm vụ và nhận thưởng trận đấu trong cùng thời điểm — số lượng tồn kho luôn được cập nhật chính xác mà không cần khóa ở tầng cơ sở dữ liệu.
