...
interface PvpBattleState {
    ...
    isP1: boolean;  // true = this client is Challenger in DB
}

// Swap damagedPlayer 1↔2 for P2's perspective
...
// Build RoundResultDto from current client's perspective (swap if needed)
...
export function usePvpBattle(sessionId: number) {
    ...
    const isP1Ref = useRef<boolean>(false); // used in callbacks because state is stale
        ...
        connectBattleHub(token, {
            ...
            onBattleEnded: (data: BattleEndedDto) => {
                ...
                const adjusted: BattleEndedDto = {
                    ...
                    eloResult: myElo,
                    p2EloResult: undefined,        // don't need p2EloResult at client
                };
