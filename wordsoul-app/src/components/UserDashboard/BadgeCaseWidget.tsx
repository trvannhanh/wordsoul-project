  ...
  return (
    <div className="bg-black/80 rounded-xl border-2 border-yellow-400 p-5 mt-6 relative overflow-hidden group hover:shadow-[0_0_15px_rgba(250,204,21,0.5)] transition-shadow">
      ...
      <div className="flex justify-between items-center mb-4 relative z-10">
        <div>
          <h2 className="font-press text-yellow-500 text-sm drop-shadow-md">JOHTO & KANTO BADGE CASE</h2>
          <p className="font-pixel text-[10px] text-gray-400 mt-1">{defeatedCount} / 16 BADGES EARNED</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-8 gap-y-3 gap-x-1 sm:gap-2 justify-items-center bg-black/50 p-3 sm:p-4 rounded-lg border border-gray-700 relative z-10">
        {Array.from({length: 16}, (_, i) => i + 1).map(order => {
          const gym = gyms.find(g => g.gymOrder === order);
          const isDefeated = gym?.status === GymStatus.Defeated;
          return (
            <div 
              key={order} 
              className={`w-8 h-8 flex items-center justify-center text-xl sm:text-2xl transition-all duration-300 ${isDefeated ? 'scale-110 drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]' : 'grayscale opacity-30'}`}
